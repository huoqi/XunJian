		...
		' Else
			...
			if crt.Screen.WaitForString ("assword:", 30) = True then
				...
				'Ask for the enable to connect on all the nodes
					if haven = "no" then 
						result = crt.Dialog.Prompt("Please enter enable password", "password", "", True)
						if result <> "" then
							enablepass = result
						end if
					end if
				...
				crt.Screen.Send "term width 1023" & VbCr 
				crt.Screen.WaitForString promptString				
				...
				crt.Screen.Send "show ism gl | begin after 4 'Num Circuits (TOTAL'" & VbCr 
				crt.Screen.WaitForString promptString
				...
				crt.Screen.Send "show redundancy | begin Switchover" & VbCr 
				crt.Screen.WaitForString promptString 
				...
				crt.Screen.Send "show system nvlog" & VbCr  
				crt.Screen.WaitForString promptString

				crt.Screen.Send "show log | ex PPPOE-5-ISM_ERR | ex AAA-6-INFO | ex SNMP-6-INFO" & VbCr
